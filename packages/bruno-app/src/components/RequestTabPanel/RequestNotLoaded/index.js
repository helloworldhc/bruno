            ...
            {!item?.error && (
              <div className="flex flex-col">
                <div className="flex items-center gap-2 px-3 py-2 title bg-yellow-50 dark:bg-yellow-900/20">
                  ...
                  <span>{t('REQUEST_TAB_PANEL.LARGE_REQUEST_MSG', "The request wasn't loaded due to its large size. Please try again with the following options:")}</span>
                </div>
                <div className="flex flex-row mt-6 items-center gap-2 w-full">
                  ...
                  >
                    {t('REQUEST_TAB_PANEL.LOAD_REQUEST', 'Load Request')}
                  </button>
                  <p>({t('REQUEST_TAB_PANEL.REGEX_APPROACH', 'Uses a regex based parsing approach')})</p>
                </div>
              </div>
            )}
