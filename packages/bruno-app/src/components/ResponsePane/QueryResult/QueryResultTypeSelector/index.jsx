...
const QueryResultTypeSelector = ({
  ...
  const header = (
    <div className="flex items-center justify-between gap-3 py-[0.35rem] px-[0.6rem]">
      <span className="text-[0.8125rem] preview-response-tab-label">{t('COMMON.PREVIEW', 'Preview')}</span>
      <ToggleSwitch
        ...
        title={selectedTab === 'preview' ? t('RESPONSE.TURN_OFF_PREVIEW', 'Turn off Preview Mode') : t('RESPONSE.TURN_ON_PREVIEW', 'Turn on Preview Mode')}
      />
    </div>
  );
