...
const CollectionItem = ({ item, collectionUid, collectionPathname, searchText, openBulkMenu }) => {
  ...
  const handleCopyItem = () => {
    ...
    toast.success(t('COMMON.ITEM_COPIED', '{{item}} copied', { item: getItemTypeLabel(item) }));
  };
  ...
  const handleCloneItem = () => {
    ...
    dispatch(cloneItem(`${item.name} copy`, sanitizeName(`${item.name} copy`), item.uid, collectionUid))
      .then(() => toast.success(t('COLLECTIONS.ITEM_CLONED', '{{item}} cloned!', { item: getItemTypeLabel(item) })))
      .catch((err) => toast.error(formatIpcError(err) || t('COLLECTIONS.CLONE_ITEM_ERROR', 'An error occurred while cloning the {{item}}', { item: getItemTypeLabel(item).toLowerCase() })));
  };
  ...
  const handlePasteItem = () => {
    ...
    dispatch(pasteItem(collectionUid, targetFolderUid))
      ...
      .catch((err) => {
        toast.error(formatIpcError(err) || t('COLLECTION.PASTE_ITEM_ERROR', 'An error occurred while pasting the item'));
      });
  };
