...
const BrunoSupport = ({ onClose }) => {
  ...
  return (
    <StyledWrapper>
      <Modal size="sm" title={t('SUPPORT.TITLE', 'Support')} handleCancel={onClose} hideFooter={true}>
        <div className="collection-options">
          <div className="mt-2">
            <a href="https://docs.usebruno.com" target="_blank" className="flex items-end">
              <IconBook size={18} strokeWidth={2} />
              <span className="label ml-2">{t('SUPPORT.DOCUMENTATION', 'Documentation')}</span>
            </a>
          </div>
          <div className="mt-2">
            <a href="https://github.com/usebruno/bruno/issues" target="_blank" className="flex items-end">
              <IconSpeakerphone size={18} strokeWidth={2} />
              <span className="label ml-2">{t('SUPPORT.REPORT_ISSUES', 'Report Issues')}</span>
            </a>
          </div>
          <div className="mt-2">
