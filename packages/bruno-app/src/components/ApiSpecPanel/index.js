...
const ApiSpecPanel = () => {
  ...
  if (!uid) {
    return <div className="p-4 opacity-50">{t('API_SPEC.NOT_FOUND', 'API Spec not found!')}</div>;
  }
  ...
  const handleOpenApiSpec = () => {
    dispatch(openApiSpec()).catch(
      (err) => console.log(err) && toast.error(t('API_SPEC.OPEN_ERROR', 'An error occurred while opening the API spec'))
    );
  };
      ...
      <div className="p-3 mb-2 w-full flex flex-row justify-between grid grid-cols-3">
        <div className="flex flex-row justify-start gap-x-4 col-span-1">
          <div className="flex w-fit items-center cursor-pointer">
            ...
            <span className="ml-2 mr-4 font-semibold">{t('API_SPEC.API_DESIGNER', 'API Designer')}</span>
          </div>
        </div>
        ...
        <div className="menu-icon pr-2 col-span-1 flex justify-end">
          <Dropdown onCreate={onDropdownCreate} icon={<MenuIcon />} placement="bottom-start">
            ...
            >
              {t('API_SPEC.CREATE_API_SPEC', 'Create API Spec')}
            </div>
            <div
              className="dropdown-item"
              onClick={(e) => {
            ...
            >
              {t('API_SPEC.OPEN_API_SPEC', 'Open API Spec')}
            </div>
          </Dropdown>
        </div>
      </div>
