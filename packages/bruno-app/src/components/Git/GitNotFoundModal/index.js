...
const GitNotFoundModal = ({ onClose }) => {
  ...
  return (
    <Portal>
      <Modal
        size="sm"
        title={t('GIT.NOT_FOUND_TITLE', 'Git Not Found')}
        handleCancel={onClose}
        hideFooter={true}
      >
        <div>
          <p>{t('GIT.NOT_FOUND_MSG', 'Git was not detected on your system. You need to install Git to proceed.')}</p>
          <p className="mt-2">
            {t('GIT.DOWNLOAD_FOR_OS', 'You can download Git for {{os}} here:', { os: osName })}
          </p>
          <p>
            <span
            ...
            >
              {t('GIT.DOWNLOAD_GIT_FOR', 'Download Git for {{os}}', { os: osName })}
            </span>
          </p>
        </div>
      </Modal>
