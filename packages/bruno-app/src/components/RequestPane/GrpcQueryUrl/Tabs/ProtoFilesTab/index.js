    ...
    <StyledWrapper>
          ...
          {invalidProtoFiles.length > 0 && (
            <div className="error-wrapper">
              <p className="error-text">
                ...
                {t('GRPC.PROTO_FILES_NOT_FOUND', 'Some proto files could not be found.')}
                {' '}
                <button
                  onClick={onOpenCollectionProtobufSettings}
                  className="error-link"
                >
                  {t('GRPC.MANAGE_PROTO_FILES', 'Manage proto files')}
                </button>
              </p>
            </div>
          )}
      ...
      {(!collectionProtoFiles || collectionProtoFiles.length === 0) && (
        <div className="empty-wrapper">
          <div className="empty-text">
            {t('GRPC.NO_PROTO_FILES', 'No proto files configured in collection settings')}
          </div>
        </div>
      )}
      ...
      <div className="button-wrapper">
        ...
        >
          ...
          {t('GRPC.BROWSE_PROTO_FILE', 'Browse for Proto File')}
        </button>
      </div>
    </StyledWrapper>
  );
