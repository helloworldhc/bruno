        ...
        <div className="schema-empty-state">
          {schemaError ? (
            <>
              ...
              <div className="empty-state-title">{t('QUERY_BUILDER.SCHEMA_LOAD_FAILED', 'Failed to Load Schema')}</div>
              ...
              <div className="empty-state-actions">
                ...
                >
                  {t('QUERY_BUILDER.TRY_AGAIN', 'Try Again')}
                </Button>
                <Button
                  variant="outline"
                  color="secondary"
                ...
                >
                  {t('QUERY_BUILDER.UPLOAD_SCHEMA', 'Upload Schema File')}
                </Button>
              </div>
            </>
          ) : (
            <>
              <div className="empty-state-title">{t('QUERY_BUILDER.NO_SCHEMA', 'No Schema Loaded')}</div>
              <div className="empty-state-description">
                {t('QUERY_BUILDER.SCHEMA_DESCRIPTION', 'Load a GraphQL schema to explore operations and build queries visually.')}
              </div>
              <div className="empty-state-actions">
                <Button
                  variant="outline"
                  ...
                  onClick={() => loadSchema('introspection')}
                >
                  {t('QUERY_BUILDER.LOAD_INTROSPECTION', 'Load from Introspection')}
                </Button>
                <Button
                  variant="outline"
                  color="secondary"
        ...
        <div className="sync-error-banner">
          ...
          <div className="sync-error-text">
            {syncError === 'multiple_operations' ? (
              <>
                <strong>{t('QUERY_BUILDER.MULTIPLE_OPERATIONS', 'Multiple operations detected')}</strong>
                <span>{t('QUERY_BUILDER.MULTIPLE_OPERATIONS_DESC', 'The Query Builder supports a single operation at a time. Combine into one operation to sync.')}</span>
              </>
            ) : null}
          </div>
        </div>
    ...
    <ErrorBoundary>
      <StyledWrapper>
        <div className="query-builder-search">
          <input
            type="text"
            placeholder={t('QUERY_BUILDER.SEARCH_OPERATIONS', 'Search operations...')}
            value={searchText}
            className="mousetrap"
                ...
                {isExpanded && !isDisabled && (
                  fields.length > 0 ? (
                    ...
                    <div className="empty-state">
                      {searchText ? t('QUERY_BUILDER.NO_MATCHING_FIELDS', 'No matching fields.') : t('QUERY_BUILDER.NO_FIELDS', 'No fields available.')}
                    </div>
                  )
                )}
              </div>
