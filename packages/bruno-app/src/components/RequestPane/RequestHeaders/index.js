...
const RequestHeaders = ({ item, collection, addHeaderText }) => {
    ...
    return [
      {
        ...
        label: t('REQUEST.INHERITED_HEADERS', 'Inherited Headers'),
      ...
      ...(isDefaultHeadersExpanded ? defaultHeaders : []),
      {
        ...
        label: t('REQUEST.REQUEST_HEADERS', 'Request Headers'),
  ...
  const getRowError = useCallback((row, index, key) => {
    ...
    if (key === 'name') {
      ...
      if (!headerNameRegex.test(row.name)) {
        return t('REQUEST.HEADER_NAME_ERROR', 'Header name cannot contain spaces or newlines');
      }
    }
    if (key === 'value') {
      if (!row.value) return null;
      if (!headerValueRegex.test(row.value)) {
        return t('REQUEST.HEADER_VALUE_ERROR', 'Header value cannot contain newlines');
      }
    }
    return null;
  }, [t]);
  ...
  const renderDefaultHeaderAction = useCallback((row) => {
    ...
    return (
      <ToolHint
        text={row.omittable
          ? t('REQUEST.HEADER_ADDED_AT_RUNTIME', 'Automatically added at runtime')
          : t('REQUEST.HEADER_CANNOT_BE_OMITTED', 'Required by HTTP, cannot be omitted')}
        ...
        if (row.rowType === ROW_TYPE.DEFAULT) {
          return (
            <div className="header-name-cell">
              ...
              {row.overridden && (
                <ToolHint
                  text={t('REQUEST.HEADER_OVERRIDDEN', 'Overridden by a request header')}
        ...
        return (
          <div className="header-name-cell">
            ...
            {row.enabled !== false
              ...
              <ToolHint
                text={t('REQUEST.HEADER_OVERRIDES_DEFAULT', 'Overrides Bruno\'s default header')}
        ...
        <div>
          {isHttpRequest && (
            ...
            >
              ...
              <span>
                {showDefaultHeaders
                  ? t('REQUEST.HIDE_INHERITED_HEADERS', 'Hide Inherited Headers')
                  : t('REQUEST.SHOW_INHERITED_HEADERS', 'Show Inherited Headers ({{count}})', { count: defaultHeaders.length })}
              </span>
            </button>
          )}
        </div>
