...
const CompatEndpointCard = ({
  ...
  const handleSaveKey = async () => {
    ...
    try {
      ...
      setShowKey(false);
      setEditing(false);
      setFeedback({ type: 'success', message: t('PREFERENCES.AI_API_KEY_SAVED', 'API key saved') });
    } catch (err) {
      setFeedback({ type: 'error', message: err.message || t('PREFERENCES.AI_API_KEY_SAVE_FAILED', 'Failed to save API key') });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (result.ok) {
        setFeedback({ type: 'success', message: t('PREFERENCES.AI_CONN_SUCCESS', 'Connection successful') });
      } else {
        setFeedback({ type: 'error', message: result.error || t('PREFERENCES.AI_CONN_FAILED', 'Connection failed') });
      }
    } catch (err) {
      setFeedback({ type: 'error', message: err.message || t('PREFERENCES.AI_CONN_FAILED', 'Connection failed') });
    } finally {
      setTesting(false);
    }
  };
  ...
  const handleAddModel = () => {
    ...
    const id = uuid();
    onAddModel({ id, modelId: trimmedId, label });
    setNewModelId('');
    setNewModelLabel('');
  };
      ...
      >
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          ...
          <div className="flex flex-col min-w-0">
            <span className="font-semibold text-[12.5px] truncate">{endpoint.name || t('PREFERENCES.AI_UNNAMED_ENDPOINT', 'Unnamed endpoint')}</span>
            {endpoint.baseURL && (
        ...
        <div className="flex items-center gap-2.5 flex-shrink-0">
          <span className={`provider-status inline-flex items-center gap-1.5 text-[11px] ${provider.configured ? 'configured' : ''}`}>
            ...
            {provider.configured
              ? `${enabledModelsCount}/${models.length} ${t('PREFERENCES.AI_MODELS_COUNT', 'models')}`
              : t('PREFERENCES.AI_NOT_CONFIGURED', 'Not configured')}
          </span>
          ...
          <div className="provider-body flex flex-col gap-3.5 px-3 pt-3 pb-3">
            ...
            <div className="grid grid-cols-2 gap-2" onClick={stopBubble}>
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  placeholder={t('PREFERENCES.AI_ENDPOINT_NAME_PLACEHOLDER', 'e.g. Ollama local')}
                  value={endpoint.name || ''}
              ...
              <div className="flex flex-col gap-1">
                <label className="key-section-label text-[11px]" htmlFor={`endpoint-baseurl-${endpoint.id}`}>
                  {t('PREFERENCES.AI_BASE_URL', 'Base URL')}
                </label>
                <input
            ...
            <div>
              <div className="key-section-label flex items-center justify-between gap-2 text-[11px] mb-1">
                <span>{t('PREFERENCES.AI_API_KEY', 'API Key')} <span className="opacity-60">({t('COMMON.OPTIONAL', 'optional')})</span></span>
              </div>
              ...
              {!editing ? (
                ...
                >
                  {provider.hasApiKey ? (
                    ...
                    <span className="key-display-mask text-xs opacity-60">{t('COMMON.NOT_SET', 'Not set')}</span>
                  )}
                  <div className="flex items-center gap-0.5">
                    <button
                      ...
                      title={endpoint.baseURL ? t('PREFERENCES.AI_TEST_CONNECTION', 'Test connection') : t('PREFERENCES.AI_SET_BASE_URL_FIRST', 'Set Base URL first')}
                      aria-label={t('PREFERENCES.AI_TEST_CONNECTION', 'Test connection')}
                    ...
                    <button
                      ...
                      disabled={pending}
                      title={provider.hasApiKey ? t('PREFERENCES.AI_REPLACE_KEY', 'Replace key') : t('PREFERENCES.AI_ADD_KEY', 'Add key')}
                      aria-label={provider.hasApiKey ? t('PREFERENCES.AI_REPLACE_KEY', 'Replace key') : t('PREFERENCES.AI_ADD_KEY', 'Add key')}
                    ...
                    {provider.hasApiKey && (
                      <button
                        ...
                        disabled={pending}
                        title={t('PREFERENCES.AI_REMOVE_KEY', 'Remove key')}
                        aria-label={t('PREFERENCES.AI_REMOVE_KEY', 'Remove key')}
                ...
                <div className="flex items-center gap-1.5" onClick={stopBubble}>
                  <div className="relative flex-1 flex items-center">
                    ...
                    <button
                      ...
                      tabIndex={-1}
                      aria-label={showKey ? t('PREFERENCES.AI_HIDE_API_KEY', 'Hide API key') : t('PREFERENCES.AI_SHOW_API_KEY', 'Show API key')}
                    >
              ...
              {pending && (
                <div className="feedback flex items-center gap-1.5 text-[11px] px-2 py-1 mt-1.5" role="status">
                  ...
                  {t('PREFERENCES.AI_SAVING_ENDPOINT', 'Saving endpoint…')}
                </div>
              )}
            ...
            <div className="flex flex-col gap-1.5" onClick={stopBubble}>
              <div className="models-label-row flex items-center justify-between text-[11px]">
                <span>{t('PREFERENCES.AI_MODELS', 'Models')}</span>
                {!provider.configured && (
                  <span className="keyless-hint flex items-center gap-1.5 text-[11px] py-1">
                    <IconAlertCircle size={12} />
                    {t('PREFERENCES.AI_SET_BASE_URL_ENABLE', 'Set a Base URL to enable')}
                  </span>
                )}
              </div>
              ...
              {models.length === 0 && (
                <div className="compat-models-empty text-[11px] px-2.5 py-2">
                  {t('PREFERENCES.AI_NO_MODELS_YET', 'No models yet. Add the model id your provider expects (e.g. gpt-4o or llama3.1:8b).')}
                </div>
              )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <input
                          ...
                          value={model.label || ''}
                          placeholder={t('PREFERENCES.AI_DISPLAY_NAME', 'Display name')}
                        ...
                        <input
                          ...
                          value={model.modelId || ''}
                          placeholder={t('PREFERENCES.AI_MODEL_ID', 'Model id')}
                        ...
                        <button
                          ...
                          title={t('PREFERENCES.AI_REMOVE_MODEL', 'Remove model')}
                          aria-label={t('PREFERENCES.AI_REMOVE_MODEL', 'Remove model')}
                        >
                          <IconTrash size={13} />
                        </button>
                      </div>
              ...
              <div className="compat-add-model flex items-center gap-1.5 mt-1">
                <input
                  ...
                  placeholder={t('PREFERENCES.AI_MODEL_ID_REQ', 'Model id (required)')}
                  value={newModelId}
                ...
                <input
                  type="text"
                  className="key-input flex-1 h-8 box-border text-xs leading-none pl-2.5 pr-2"
                  placeholder={t('PREFERENCES.AI_LABEL_OPT', 'Label (optional)')}
                  value={newModelLabel}
            ...
            <div className="flex justify-end pt-1" onClick={stopBubble}>
              ...
              >
                <IconTrash size={12} />
                {t('PREFERENCES.AI_REMOVE_ENDPOINT', 'Remove endpoint')}
              </button>
            </div>
          </div>
        </div>
