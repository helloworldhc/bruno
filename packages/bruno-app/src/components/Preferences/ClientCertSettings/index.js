...
const ClientCertSettings = () => {
  ...
  const persistCerts = (updatedCerts) => {
    ...
    ).catch(() => toast.error(t('PREFERENCES.SAVE_ERROR', 'Failed to save preferences')));
  };
      ...
      {showAddCertModal && (
        ...
        >
          <div className="text-xs mb-4 text-muted">
            {t('PREFERENCES.ADD_CLIENT_CERT_DESC', 'The certificate and key files are stored as absolute paths and apply to every collection.')}
          </div>
